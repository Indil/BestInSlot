...
function BestInSlot:GetItemTooltip(itemid, difficulty, frame, itemlink)
  ...
  if self.options.DEBUG then
    ...
    GameTooltip:AddLine("boss n°: "..tostring(item.bossid))
	--GameTooltip:AddLine("boss n°: "..tostring(item.nboss))
    GameTooltip:AddLine("link: "..tostring(item.link))
    GameTooltip:AddLine("difficulty: "..tostring(item.difficulty))
    if itemtype ~= RELIC then --added implement relic items
	  GameTooltip:AddLine("equipslot: "..tostring(item.equipSlot))
	else
	  GameTooltip:AddLine("equipslot: "..tostring(RELIC))
	end
	if self:HasItem(itemid, difficulty, true) then
	  GameTooltip:AddLine("gotit: "..tostring(self:HasItem(itemid, difficulty, true)) and "true") -- ToDo if equipped or inventory
	end
    if item.misc then
  ...
  if self.db.char.options.showBiSTooltip and (self.db.char.options.tooltipCombat or not UnitAffectingCombat("player")) then
    ...
    if link then
      ...
      if itemId then
        ...
        instanceID = tonumber(self:GetDifficultyIdForDungeon(instanceID, nil, true) or 1)
        local bisInfo
        ...
        if bisInfo and type(bisInfo) == "table" then
          ...
          if isBiS then
			...
			--print(tostring(itemId).." - "..tostring(instanceID))
            if self:HasItem(itemId, instanceID, true) then -- fixed issue on getting out instancedifficulty from equip and bags
        ...
        if item and (self.db.char.options.tooltipSource or self.options.DEBUG) then
          if item.multiplesources then
            if (GetLocale() == "itIT") then
				tooltip:AddLine("Fonti:")
			else
				tooltip:AddLine(SOURCES..":") -- wrong global translation (itIT)
			end
            for dungeonname,v in pairs(item.multiplesources) do
              for bossid in pairs(v) do
				dung = data.instances[dungeonname].description -- changed dungeonname to full name
                tooltip:AddLine(("%s%s %s(%s)|r"):format(self.colorHighlight, self:GetDescription(self.BOSS, dungeonname, bossid), self.colorNormal, dung))
              end
            end
          elseif item.dungeon and item.bossid then
            tooltip:AddLine(("%s %s%s"):format(SOURCE, self.colorSource, self:GetDescription(self.BOSS, item.dungeon, item.bossid)))
          elseif item.misc then
            tooltip:AddLine(("%s %s%s"):format(SOURCE, self.colorLegendary, item.misc))
          end
        end
...
local function GetFrame()
  ...
  frame:SetStatusText((L["Created by %s, "]):format(BestInSlot.Author)..(L["currently developed by %s"]):format(BestInSlot.Developer))
  local FrameBackdrop = {
